"""
Simple debugging script to test RAG functionality
"""
import os
import sys
...
print(f"COLLECTION_NAME: {os.getenv('QDRANT_COLLECTION_NAME', os.getenv('COLLECTION_NAME', 'default-collection'))}")
...
def test_connection():
    ...
    try:
        ...
        print("[SUCCESS] Cohere client initialized successfully")
    except Exception as e:
        print(f"[ERROR] Error initializing Cohere client: {e}")
        return False
    ...
    try:
        ...
        print(f"[SUCCESS] Qdrant connection successful. Available collections: {[col.name for col in collections.collections]}")
...
def test_query():
    ...
    if results and results.points:
        print(f"Number of results retrieved: {len(results.points)}")
        for i, result in enumerate(results.points[:2]):  # Show first 2 results
            print(f"Result {i+1}:")
